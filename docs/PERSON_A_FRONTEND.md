...
**Key behaviors:**
- SQL/Code blocks: collapsible (default collapsed), syntax highlighted, copy button
...
1. ☐ Welcome screen renders with upload zone
2. ☐ File upload triggers callback with file object
3. ☐ Chat interface shows messages
4. ☐ User messages appear on right, AI on left
5. ☐ Charts render (bar, line, pie) with sample data
6. ☐ Confidence ring animates on mount
7. ☐ Code blocks show SQL/Python with syntax highlighting
8. ☐ Sidebar shows file info
9. ☐ PDF export button triggers download
10. ☐ Responsive: no horizontal scroll, no overflow issues
...
|---|---|---|
| 1-2 | Scaffold + TailwindCSS + index.css + App shell | ☐ |
| 2-3 | api.js + useChat.js + WelcomeScreen + FileUpload | ☐ |
| 3-5 | ChatInterface + MessageBubble (text only) | ☐ |
| 5-6 | ChartRenderer (Recharts) + CodeBlock | ☐ |
| 6-7 | ConfidenceScore + DataPreview | ☐ |
| 7-8 | Sidebar + SemanticLayerEditor | ☐ |
| 8-10 | Integration with Person B's backend | ☐ |
| 10-12 | Polish: animations, loading states, error handling, responsive | ☐ |
